...
public class StudentStreamService {
    ...
    public Pair<Long, Long> calcQuasiSumOfArithmeticProgressionFrom1to1MUsingParallelStreamReduce(int repeats) {
        logger.info("Method calcQuasiSumOfArithmeticProgressionFrom1to1MusingParallelStreamReduce(int repeats={}) has been invoked.",
                repeats);
        long result = 0;
        long time0 = System.currentTimeMillis();
        for (int i = 0; i < repeats; i++) {
